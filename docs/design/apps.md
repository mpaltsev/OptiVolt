...
- Catalog bundled at build via `manifest.json` (pick market).
...
- Usage parsers: one module per `format_id`; first sample: `il_iec`.
- Host: Cloudflare Pages / GitHub Pages.

## Extract tool (`tools/extract`)

- Scope: **private supplier plans only** (not official incumbent hand tables).
...
- Never invent official flat/TOU rates; never pick the winner for users.
